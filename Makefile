...
bundled = $(dir_build)/rebootpatch.h $(dir_build)/emunandpatch.h $(dir_build)/k11modulespatch.h $(dir_build)/svcGetCFWInfopatch.h $(dir_build)/twl_k11modulespatch.h \
		  $(dir_build)/arm9_exceptions.h $(dir_build)/arm11_exceptions.h $(dir_build)/injector.h $(dir_build)/loader.h $(dir_build)/createCustomThread.h
.PHONY: all
...
$(dir_build)/createCustomThread.h: $(dir_patches)/createCustomThread.s
	@mkdir -p "$(@D)"
	@armips $<
	@bin2c -o $@ -n createCustomThread $(@D)/createCustomThread.bin
